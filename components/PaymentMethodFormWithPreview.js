// components/PaymentMethodFormWithPreview.js - FORMULÁRIO COM PRÉVIA
...
export default function PaymentMethodFormWithPreview({ visible, paymentMethod, onClose, onSaved }) {
  ...
  const validateForm = () => {
    ...
    if (!formData.icon) {
      newErrors.icon = 'Escolha um ícone para a forma de pagamento';
    }
  ...
  const handleSave = async () => {
    ...
    if (!user) {
      Alert.alert('Erro', 'Você precisa estar logado para gerenciar formas de pagamento.', [{ text: 'Entendi' }]);
      return;
    }
    ...
    try {
      ...
      if (paymentMethod?.id) {
        ...
        if (existingMethod) {
          Alert.alert('Atenção', 'Já existe uma forma de pagamento com este nome.', [{ text: 'Entendi' }]);
          return;
        }
      ...
      Alert.alert('Erro', 'Não foi possível salvar a forma de pagamento. Tente novamente.', [{ text: 'Entendi' }]);
    } finally {
          ...
          <View style={styles.modalContainer}>
              ...
              <View style={styles.headerContent}>
                <View style={styles.headerLeft}>
                  ...
                  <Text style={styles.subtitle}>
                    {paymentMethod?.id ? 'Atualize as informações' : 'Preencha os dados'}
                  </Text>
                </View>
            ...
            >
              ...
              <View style={styles.fieldContainer}>
                ...
                <Text style={styles.charCounter}>{formData.name.length}/30</Text>
              </View>
              ...
              <View style={styles.fieldContainer}>
                ...
                >
                  {AVAILABLE_PAYMENT_ICONS.map((icon, index) => (
                    <TouchableOpacity
                      key={index}
                      style={[
                        styles.iconOption,
            ...
            <View style={styles.footer}>
              ...
              >
                ...
                <Text style={styles.saveButtonText}>
                  {paymentMethod?.id ? 'Atualizar' : 'Salvar'}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
